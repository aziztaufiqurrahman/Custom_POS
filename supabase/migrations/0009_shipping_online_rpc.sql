-- =============================================================================
-- 0009_shipping_online_rpc.sql
-- - kolom shipping_cost (ongkos kirim) di transactions — TIDAK masuk pendapatan
-- - kolom total_gofood / total_shopeefood di cash_sessions
-- - update RPC create_sale (p_shipping_cost + total online), void/refund, analytics
-- =============================================================================
...
alter table public.cash_sessions
  add column if not exists total_gofood numeric(14,2) not null default 0,
  add column if not exists total_shopeefood numeric(14,2) not null default 0;

-- ---------------------------------------------------------------------------
-- create_sale (tambah p_shipping_cost + ringkasan shift utk gofood/shopeefood)
-- ---------------------------------------------------------------------------
drop function if exists public.create_sale(uuid, jsonb, jsonb, numeric, text, text, text);
...
begin
  ...
  -- Ongkos kirim TIDAK menambah grand_total (bukan pendapatan). Pembayaran tetap
  -- hanya untuk nilai barang (v_grand).
...
-- ---------------------------------------------------------------------------
-- void_sale & refund_sale: sesuaikan ringkasan shift utk gofood/shopeefood
-- ---------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    insert into public.stock_movements (product_id, type, qty_change, stock_after, reference_id, note, created_by)
      values (v_item.product_id, 'void', v_item.qty, v_stock_after, p_transaction_id, 'Void ' || v_trx.code, v_uid);
  end loop;
  ...
  insert into public.audit_logs (actor_id, action, entity, entity_id, metadata)
  values (v_uid, 'sale.void', 'transaction', p_transaction_id, jsonb_build_object('code', v_trx.code, 'reason', p_reason));
...
begin
  ...
  loop
    update public.products set stock = stock + v_item.qty
      where id = v_item.product_id returning stock into v_stock_after;
    insert into public.stock_movements (product_id, type, qty_change, stock_after, reference_id, note, created_by)
      values (v_item.product_id, 'refund', v_item.qty, v_stock_after, p_transaction_id, 'Refund ' || v_trx.code, v_uid);
  end loop;
  ...
  insert into public.audit_logs (actor_id, action, entity, entity_id, metadata)
  values (v_uid, 'sale.refund', 'transaction', p_transaction_id, jsonb_build_object('code', v_trx.code, 'reason', p_reason));
  return jsonb_build_object('transaction_id', p_transaction_id, 'status', 'refunded');
end; $$;

-- ---------------------------------------------------------------------------
-- dashboard_analytics: tambah shipping_total (ongkos kirim, informasional)
-- ---------------------------------------------------------------------------
