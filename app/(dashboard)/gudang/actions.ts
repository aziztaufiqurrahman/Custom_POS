...
export async function recordWastage(raw: unknown): Promise<WhResult> {
  const { profile } = await getSession();
  if (!profile || !isAdmin(profile)) return { error: "Tidak berwenang" };
  ...
  // Master admin (pusat) mencatat langsung; manajer cabang WAJIB persetujuan
  // admin pusat sebelum stok berkurang (anti-fraud).
  ...
  const { data, error } = await supabase.rpc("request_wastage", {
    p_branch_id: branch,
    p_reason: parsed.data.reason,
    p_items: parsed.data.items as unknown as Json,
  });
  ...
  await notifyRisky({
    branchId: branch,
    title: "Barang rusak menunggu persetujuan",
    body: `${profile.full_name}: ${parsed.data.reason} (${r.code}). Perlu persetujuan admin sebelum stok dikurangi.`,
  });
  revalidatePath("/gudang");
  revalidatePath("/approvals");
  return { success: true, id: r.id, code: r.code, pending: true };
}
